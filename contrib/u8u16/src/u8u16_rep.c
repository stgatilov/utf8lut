...
long long do_UTF8toUTF16(const char *inputData, long long inputSize, char *outputData) {
  ...
  const char *inputEnd = inputData + inputSize;
  char *outputStart = outputData;
  memcpy(&UTF8_buffer, inputData, BUFFER_SIZE);     //note: crashes on small files!
  inputData += BUFFER_SIZE;
  ...
  while (chars_read > 0) {
    ...
    memcpy(outputData, &UTF16_buffer, 2 * UTF16_chars);
    outputData += 2 * UTF16_chars;
    ...
    int readbytes = BUFFER_SIZE-inbytes_left;
    if (readbytes > inputEnd - inputData)
        readbytes = inputEnd - inputData;
    memcpy(&UTF8_buffer[inbytes_left], inputData, readbytes);
    inputData += readbytes;
    chars_read = readbytes;
    chars_read += inbytes_left;
  }
#ifdef STD_ICONV
  iconv_close(cd);
#endif
  return outputData - outputStart;
//  fclose(infile);
//  fclose(outfile);
}
...
//=================================
  ...
  ReadFileContents(argv[1], &inputData, &inputSize);
  char *outputData = malloc(inputSize * 2);
  long long outputSize = 0;
  int times = atoi(argv[3]);
  for (int i = 0; i < times; i++)
      outputSize = do_UTF8toUTF16(inputData, inputSize, outputData);
  WriteFileContents(argv[2], outputData, outputSize);
  uint64_t endTicks = get_ticks();
  double ticksPerElem = double(endTicks - startTicks) / times / inputSize;
